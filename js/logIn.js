...
let GetUserList = function () {
    fetch('http://localhost:3000/user/playlist?uid=' + user.id + '')
        .then(res => { return res.json() }).then(data => {
            ...
            let remove1 = list1.querySelector('#remove') //bug修正，详见readme
            let remove2 = list2.querySelector('#remove')
            list1.removeChild(remove1)
            list2.removeChild(remove2)
        })
}
