...
let commentF = function () {
    fetch('http://localhost:3000/comment/music?id=' + localStorage.songId + '')
        .then(res => { return res.json() }).then(data => {
            comment.querySelector('.cNum').innerText = '全部评论(' + data.total + ')'
            //热评
            for (let i = 0; i < data.hotComments.length; i++) {
                ...
                comment.querySelector('.Cright span').innerText = '' + data.hotComments[i].likedCount + ''
            }
            ...
            for (let i = 0; i < data.comments.length; i++) {
                ...
                comment.querySelector('.Cright span').innerText = '' + data.comments[i].likedCount + ''
            }
            //bug修正
...
document.querySelector('.search').onblur = function () { //搜索框失去焦点，消失
    setTimeout("hotSearch.style.display = 'none'", 100)
}
//获取默认搜索词
